// �¼��⣺node:sqlite �洢 + append-only + �ع� + ��֧��MD 1.4 / 2.4��
...
export class EventStore {
  db: DatabaseSync;
  /** �����棺ÿ������¼�������ͶӰ״̬��append �����ƽ���rollback/Ǩ��ʧЧ������׼������MD ��6/��14.3 */
  ...
  private init() {
    ...
    `);
    try { this.db.exec('ALTER TABLE worlds ADD COLUMN max_tier TEXT NOT NULL DEFAULT \'open\''); } catch { /* �Ѵ��� */ }
    try { this.db.exec('ALTER TABLE proposals ADD COLUMN base_seq INTEGER'); } catch { /* �Ѵ��� */ }
  ...
  // ---------- chapters��Ԫ������⣻������ Markdown �ļ����� server ������MD 1.4�� ----------
  ...
  stateAt(worldId: string, upto?: { worldTime?: number; sequence?: number }): WorldState {
    // �� upto����ͶӰ���棨append ��ǰ������O(1) ���أ��� upto ����ʷ��ѯ����ȫ���ط�
    if (!upto) {
  ...
  /** ���ݷּ�����ȡС��MD 5.1����min(user, card/pack, global)����;ֻ�ս� */
  ...
  /** �ع� = �ָ� prev_id ���ı�Ϊ��ͷ������ɾ����ʷ��MD 1.2 ԭ�� */
  ...
  // ---------- schema Ǩ�ƣ�MD ��14.3��v1��v2 ������ֻ�Ӳ���ԭ���ʾ��ӳ�䣩 ----------

  /** �� v1 ���¼�Ǩ�Ƶ� v2��kind ������һ�����ݵȣ��� v2 ���в����� */
...
function summarize(e: WorldEvent): string {
  ...
  switch (e.kind) {
    case 'char.create': return `${p.name} �ǳ�`;
    case 'char.death': return `${p.id} ����`;
    case 'char.revive': return `${p.id} ����`;
    case 'relation.set': return `${p.from} ��${p.type}�� ${p.to}`;
    case 'relation.end': return `��ϵ ${p.id} ����`;
    case 'fact.set': return `${p.key} = ${p.value}`;
    case 'location.move': return `${p.charId} �� ${p.place}`;
    case 'item.create': return `��Ʒ ${p.name}`;
    case 'item.transfer': return `${p.id} �� ${p.holder}`;
    case 'foreshadow.plant': return `���ʣ�${String(p.description).slice(0, 20)}`;
    case 'foreshadow.recover': return `���շ��� ${p.id}`;
