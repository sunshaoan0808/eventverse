...
/** 人物名白名单：全书频次 + 姓氏感知裁剪 + 角色词黑名单（防句式碎片与幻觉名）。LLM 与启发式共用。 */
const COMPOUND_SURNAMES = ['令狐', '欧阳', '慕容', '司徒', '诸葛', '司马', '上官', '皇甫', '长孙', '宇文', '尉迟', '赫连', '东方', '独孤', '南宫', '西门', '完颜', '呼延', '闻人'];
const SINGLE_SURNAMES = new Set('李王张刘陈杨赵黄周吴徐孙胡朱高林何郭马罗梁宋郑谢韩唐冯于董萧程曹袁邓许傅沈曾彭吕苏卢蒋蔡贾丁魏薛叶阎余潘杜戴夏钟汪田任姜范方石姚谭廖邹熊金陆郝孔白崔康毛邱秦丘虚江史顾侯邵孟龙万段雷钱汤尹黎易常武乔贺赖龚文庞樊兰殷施陶洪翟安颜倪严牛温季章鲁葛伍韦申尤毕聂丛焦向柳邢路岳齐梅莫庄辛管祝左涂谷祁时舒耿牟卜詹关苗凌费纪靳盛童欧甄项曲成游裴席卫查屈鲍霍穆隋甘单包司宁蒲窦冉翁岑'.split('').filter(c => c));
const ROLE_NAMES = new Set(['师娘', '师父', '师傅', '师兄', '师姐', '师弟', '师妹', '老师', '同学', '大叔', '大姐', '大哥', '老二', '老三', '老大', '班长', '裁判', '校警', '老板', '经理', '老头', '新生', '兄弟', '记者', '司机', '医生', '门卫', '众人', '大家', '有人', '他们', '她们', '我们', '自己', '心里', '急忙', '低声', '师娘']);
const LEADING_BAD = /^[对着跟和与向给被让把冲朝找从往叫拉推带扶望看听想觉得使他她它不没就还都也再到在说很太真好若便即就才只又候脱提低急心忽赶上有刚才]/;
const TRAILING_BAD = /[知道经话了吗呢吧的就行去来在地说着了地得出而以乎者只会都也还再又忙声快低娘忽赶然原先便里心]/;
...
export function isPlausibleCharName(name: string, fullText?: string): boolean {
